...
@Component({
  selector: 'app-view-purchase-returns-product-details',
  templateUrl: './view-purchase-returns-product-details.component.html',
  styleUrls: ['./view-purchase-returns-product-details.component.css']
})
export class ViewPurchaseReturnsProductDetailsComponent implements OnInit {
  allsupplier: any[] = [];
  allProducts: any[] = [];
  ...
  returnId = 0;
  viewPurchaseReturnProductDetailsForm!: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.viewPurchaseReturnProductDetailsForm = this.formBuilder.group({
      supplierId: ['',],
      returnDate: ['',],
    });
    ...
    if (this.productDetails) {
      this.viewPurchaseReturnProductDetailsForm.patchValue({
  ...
  getReturnById(){
    ...
    this.api.getPurchaseReturnById(this.returnId).subscribe({
      next:(res)=> {  
        ...
        this.viewPurchaseReturnProductDetailsForm.controls['supplierId'].setValue(res.data.supplierId);
        const formattedDate = this.formatDate(res.data.returnDate);
        this.paymentDetails = res.data.paymentDetails;
        this.viewPurchaseReturnProductDetailsForm.controls['returnDate'].setValue(formattedDate);
...
getProductBySupplierId(supplierId: number) {
 ...
 onMatOptionClicked(product:any) {
  ...
  let supplierId= Number(this.viewPurchaseReturnProductDetailsForm.controls['supplierId'].value);
  this.allProducts = this.allProducts.filter(p => p !== product);
  let tempProduct = {
...
onClickDeleteProductList(product: any) {
  ...
  continueToPaymentDetails(){
    ...
    if(this.viewPurchaseReturnProductDetailsForm.valid){
      
      this.productDetails = {
        supplierId: this.viewPurchaseReturnProductDetailsForm.value.supplierId,
       totalReturnBill: this.calculateTotalReturnAmount(),
       returnDate: this.viewPurchaseReturnProductDetailsForm.value.returnDate,
  ...
  this.router.navigate(['/view-purchase-return-payment-details']);
  }
