...
@Component({
  selector: 'app-view-purchase-product-details',
  templateUrl: './view-purchase-product-details.component.html',
  styleUrls: ['./view-purchase-product-details.component.css']
})
export class ViewPurchaseProductDetailsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['productName', 'batchNo', 'expiryDate','packOf','mrpPerPack','unitPrice', 'quantity', 'totalMrp','totalBill'];
  ...
  ngOnInit(): void {
    ...
    this.setFilesToUI(this.files)
      ...
      debugger
      this.purchaseId = this.activateRouter.snapshot.queryParams['id']
  ...
  continueToPaymentDetails(){
    ...
    this.router.navigate(['/view-purchases-payment-details']);
  }
}
